...
added_params.unpreferredRoutes=this.unpreferredRoutes;var data={method:'GET',url:this.url,waitMsg:this.locale.tripPlanner.labels.submitMsg,params:added_params}
...
this.unpreferredRoutes=params.unpreferredRoutes;if(params.routerId)
...
{var vectors=new Array();var endIndex=this.m_fromStore.getCount()-1;for(var i=0;i<=endIndex;i++){var from=this.m_fromStore.getAt(i);var leg=this.m_legStore.getAt(i);var mode=from.get('mode');if(mode==='WALK'||mode==='BICYCLE'||mode==='TRANSFER'){var geoLine=new OpenLayers.Feature.Vector(leg.get('legGeometry'),null,otp.util.OpenLayersUtils.BLACK_STYLE);var newLine=otp.util.OpenLayersUtils.makeStraightLine(from,this.m_toStore.getAt(i));vectors.push(geoLine);}}
...
otp.namespace("otp.planner");otp.planner.TopoRendererStatic={map:null,panel:null,extWrapper:null,mainContainerDiv:null,axisDiv:null,terrainContainerDiv:null,terrainDiv:null,previewDiv:null,terrainPct:0.8,axisWidth:40,nonBikeLegWidth:150,terrainCursor:null,currentLeft:0,currentMouseRect:null,markerLayer:null,locationPoint:null,locationMarker:null,legInfoArr:null,nonBikeWalkLegCount:null,minElev:null,maxElev:null,totalDistance:null,THIS:null,initialize:function(config)
{otp.configure(this,config);otp.planner.TopoRendererStatic.THIS=this;this.THIS=this;},processItinerary:function(itin){this.legInfoArr=new Array();this.nonBikeWalkLegCount=0;this.minElev=100000;this.maxElev=-1000;this.totalDistance=0;for(var li=0;li<itin.m_legStore.getTotalCount();li++){var leg=itin.m_legStore.getAt(li);var legInfo=new Array();this.legInfoArr.push(legInfo);legInfo.leg=leg;if(leg.get('mode')!="BICYCLE"&&leg.get('mode')!="WALK"){this.nonBikeWalkLegCount++;continue;}
var steps=leg.data.steps;var firstElev=0,lastElev=0;for(var si=0;si<steps.length;si++){this.totalDistance+=steps[si].distance;if(typeof steps[si].elevation=='undefined'){continue;}
var elevArr=steps[si].elevation.split(",");for(var ei=1;ei<elevArr.length;ei+=2){var elevFt=elevArr[ei]*3.2808399;if(elevFt<this.minElev){this.minElev=elevFt;}
if(elevFt>this.maxElev){this.maxElev=elevFt;}
if(firstElev==0&&elevFt>0)firstElev=elevFt;if(elevFt>0)lastElev=elevFt;}}
legInfo.firstElev=firstElev;legInfo.lastElev=lastElev;}},draw:function(itin,tree){this.processItinerary(itin);var height=this.panel.getHeight();width=this.map.map.size.w;if(height==0)height=180;this.render(width,height);this.extWrapper=new Ext.Panel({contentEl:this.mainContainerDiv,layout:'fit',});},redraw:function(){this.panel.remove(this.extWrapper);var width=this.panel.getWidth();var height=this.panel.getHeight();this.render(width,height);this.extWrapper=new Ext.Panel({contentEl:this.mainContainerDiv,layout:'fit',});this.panel.add(this.extWrapper);this.panel.doLayout();this.postLayout();},postLayout:function()
{var this_=this;this.extWrapper.on('resize',function(el){this_.redraw();});},render:function(width,height){var this_=this;this.currentLeft=0;this.currentMouseRect=null;this.markerLayer=null;this.locationPoint=null;this.locationMarker=null;var res=Math.floor(this.totalDistance/5000);if(res<5)res=5;var terrainWidth=(this.totalDistance*3.2808399)/res+this.nonBikeWalkLegCount*this.nonBikeLegWidth;var showPreview=(terrainWidth>width);var terrainHeight=showPreview?height*this.terrainPct:height;var previewHeight=height-terrainHeight;var elevSpan=this.maxElev-this.minElev;var elevInterval=Math.floor((elevSpan/terrainHeight)*.5)*100;if(elevInterval<100)elevInterval=100;this.minElev=elevInterval*Math.floor(this.minElev/elevInterval);this.maxElev=elevInterval*Math.ceil(this.maxElev/elevInterval);this.createContainerDivs(width,height,terrainWidth,showPreview);var axisCanvas=Raphael(this.axisDiv);var terrainCanvas=Raphael(this.terrainDiv,terrainWidth,terrainHeight);this.terrainCursor=terrainCanvas.rect(-10,0,1,terrainHeight).attr({fill:'#000',stroke:'none',opacity:.75});terrainCanvas.rect(0,0,terrainWidth,terrainHeight).attr({fill:'90-rgb(135,206,255)-rgb(204,245,255)',stroke:'none'});axisCanvas.rect(0,0,this.axisWidth,terrainHeight).attr({fill:'90-rgb(135,206,255)-rgb(204,245,255)',stroke:'none'});var d,rect;var subDivisions=(this.maxElev-this.minElev)/elevInterval;var subDivHeight=terrainHeight/subDivisions;for(d=0;d<=subDivisions;d++){var textY=subDivHeight*d;axisCanvas.rect(0,textY,this.axisWidth,1).attr({fill:'white',stroke:'none'});terrainCanvas.rect(0,textY,terrainWidth,1).attr({fill:'white',stroke:'none'});if(d==0)textY+=6;if(d==subDivisions)textY-=6;axisCanvas.text(this.axisWidth-3,textY,(this.maxElev-d*elevInterval)+"'").attr({fill:'black','font-size':'12px','font-weight':'bold','text-anchor':'end'});}
...
for(var m=0;m<mouseRects.length;m++){mouseRects[m].toFront();}
if(showPreview){var previewCanvas=Raphael(this.previewDiv,width,previewHeight);var previewPathStr="M 0 "+previewHeight+" ";for(var p=0;p<previewXCoords.length;p++){previewPathStr+="L "+previewXCoords[p]+" "+previewYCoords[p]+" ";}
previewPathStr+="L "+width+" "+previewHeight+" z";previewCanvas.path(previewPathStr).attr({stroke:'none',fill:'gray'});var pctVisible=(width-this.axisWidth)/terrainWidth;var sliderWidth=pctVisible*width;var slider=previewCanvas.rect(this.currentLeft,3,sliderWidth,previewHeight-4).attr({fill:'#bbb','fill-opacity':.5,stroke:'#000','stroke-width':2});var rightArrow=previewCanvas.path(["M",this.currentLeft+sliderWidth+6,previewHeight*.25,"L",this.currentLeft+sliderWidth+18,previewHeight*.5,"L",this.currentLeft+sliderWidth+6,previewHeight*.75,"z"]).attr({fill:'#000',stroke:'none',opacity:0});var leftArrow=previewCanvas.path(["M",this.currentLeft-6,previewHeight*.25,"L",this.currentLeft-18,previewHeight*.5,"L",this.currentLeft-6,previewHeight*.75,"z"]).attr({fill:'#000',stroke:'none',opacity:0});slider.mouseover(function(event){this.animate({fill:"#eee"},500);rightArrow.animateWith(this,{opacity:1},500);leftArrow.animateWith(this,{opacity:1},500);});slider.mouseout(function(event){this.animate({fill:"#bbb"},500);rightArrow.animateWith(this,{opacity:0},500);leftArrow.animateWith(this,{opacity:0},500);});var start=function(){this.ox=this.attr("x");},move=function(dx,dy){var nx=this.ox+dx;if(nx<0)nx=0;if(nx>width-sliderWidth)nx=width-sliderWidth;this_.currentLeft=Math.round(-(nx/width)*terrainWidth);this_.terrainDiv.style.left=this_.currentLeft+'px';var ndx=nx-this.attr("x");this.attr({x:nx});rightArrow.translate(ndx,0);leftArrow.translate(ndx,0);},up=function(){};slider.drag(move,start,up);}},createContainerDivs:function(width,height,terrainWidth,showPreview){var upperHeight=showPreview?height*this.terrainPct:height;;var lowerHeight=height-upperHeight;this.axisDiv=document.createElement('div');this.axisDiv.style.position='absolute';this.axisDiv.style.top='0px';this.axisDiv.style.left='0px';this.axisDiv.style.height=upperHeight+'px';this.axisDiv.style.width=this.axisWidth+'px';this.terrainContainerDiv=document.createElement('div');this.terrainContainerDiv.style.overflow='hidden';this.terrainContainerDiv.style.position='absolute';this.terrainContainerDiv.style.top='0px';this.terrainContainerDiv.style.left=this.axisWidth+'px';this.terrainContainerDiv.style.height=upperHeight+'px';this.terrainContainerDiv.style.width=(width-this.axisWidth)+'px';this.terrainDiv=document.createElement('div');this.terrainDiv.style.position='absolute';this.terrainDiv.style.top='0px';this.terrainDiv.style.left='0px';this.terrainDiv.style.height=upperHeight+'px';this.terrainDiv.style.width=terrainWidth+'px';if(showPreview){this.previewDiv=document.createElement('div');this.previewDiv.style.position='absolute';this.previewDiv.style.top=upperHeight+'px';this.previewDiv.style.left='0px';this.previewDiv.style.height=lowerHeight+'px';this.previewDiv.style.width=width+'px';}
this.mainContainerDiv=document.createElement('div');this.mainContainerDiv.appendChild(this.axisDiv);this.mainContainerDiv.appendChild(this.terrainContainerDiv);this.terrainContainerDiv.appendChild(this.terrainDiv);if(showPreview)this.mainContainerDiv.appendChild(this.previewDiv);},pointAlongLineString:function(ls,d){var points=ls.components;if(d<=0)return points[0];for(var i=0;i<points.length-1;i++){var segLen=points[i].distanceTo(points[i+1]);if(d<=segLen){var x=points[i].x+(d/segLen*(points[i+1].x-points[i].x));var y=points[i].y+(d/segLen*(points[i+1].y-points[i].y));return new OpenLayers.Geometry.Point(x,y);}
d-=segLen;}
...
if(typeof(otp)=="undefined"||otp==null)otp={};if(typeof(otp.config)=="undefined"||otp.config==null)otp.config={};if(typeof(otp.config.locale)=="undefined"||otp.config.locale==null)otp.config.locale=otp.locale.English;otp.osm_att="Map data © <a href='http://www.openstreetmap.org/' target='_blank'>OpenStreetMap</a> contributors, <a href='http://creativecommons.org/licenses/by-sa/2.0/' target='_blank'>CC BY-SA</a>.  ";otp.ol_rez=[156543.03390000000945292413,78271.51695000000472646207,39135.75847500000236323103,19567.87923750000118161552,9783.93961875000059080776,4891.96980937500029540388,2445.98490468750014770194,1222.99245234375007385097,611.49622617187503692548,305.74811308593751846274,152.87405654296875923137,76.43702827148437961569,38.21851413574218980784,19.10925706787109490392,9.55462853393554745196,4.77731426696777372598,2.38865713348388686299,1.19432856674194343150,0.59716428337097171575,0.29858214168548585787,0.14929107084274292894,0.07464553542137146447,0.03732276771068573223,0.01866138385534286612,0.00933069192767143306];otp.config_defaults={routerId:null,locale:otp.config.locale,metricsSystem:otp.config.locale.config.metricsSystem,planner:{url:null,printUrl:"print.html",maxTransfers:null,appName:"OpenTripPlanner Map",options:{showElevationGraph:true,showBikeshareMode:true,showTrainMode:true,showWheelchairForm:true,showIntermediateForms:true,showStopCodes:true,showAgencyInfo:true,showFareInfo:true,showReverseButton:true,showEditButton:true,showPrintButton:true,showLinksButton:true,showLayerSwitcher:true,setMaxExtentToDefault:true,useOptionDependencies:true,useRouteLongName:false,appendGeocodeName:true,OPTIONS_NOTE:"THIS IS A STRUCTURE USED TO CUSTOMIZE THE TRIP FORMS AND OTHER BEHAVIORS"},itineraryMessages:{icon:null,transit:"This is an Itinerary Message test...",transit:null,bus:null,train:null,bicycle:null,bicycle_transit:null,walk:null},linkTemplates:[{name:otp.config.locale.tripPlanner.link.text,url:'index.html#/'+otp.planner.ParamTemplate},{name:otp.config.locale.tripPlanner.link.trip_separator,separator:true},{name:otp.config.locale.tripPlanner.link.google_transit,url:otp.config.locale.tripPlanner.link.google_domain+'/maps?<tpl if="arriveBy == \'Arrive\'">ttype=arr&</tpl>date={date}&time={time}&daddr={toLat},{toLon}&saddr={fromLat},{fromLon}&ie=UTF8&dirflg=r'},{name:otp.config.locale.tripPlanner.link.bike_separator,separator:true},{name:otp.config.locale.tripPlanner.link.google_bikes,url:otp.config.locale.tripPlanner.link.google_domain+'/maps?daddr={toLat},{toLon}&saddr={fromLat},{fromLon}&ie=UTF8&dirflg=b'},{name:otp.config.locale.tripPlanner.link.walk_separator,separator:true},{name:otp.config.locale.tripPlanner.link.google_walk,url:otp.config.locale.tripPlanner.link.google_domain+'/maps?daddr={toLat},{toLon}&saddr={fromLat},{fromLon}&ie=UTF8&dirflg=w'}],geocoder:{enabled:false,url:"/geocoder/geocode",addressParamName:"address"},fromToOverride:new Ext.Template('<div class="mapHelp">'+otp.config.locale.config.rightClickMsg+'</div>')},map:{defaultExtent:"automatic",options:{projection:new OpenLayers.Projection("EPSG:900913"),displayProjection:new OpenLayers.Projection("EPSG:4326"),numZoomLevels:20,controls:[]},baseLayer:[new OpenLayers.Layer.OSM("Mapbox Streets",["http://a.tiles.mapbox.com/v3/mapbox.mapbox-streets/${z}/${x}/${y}.png","http://b.tiles.mapbox.com/v3/mapbox.mapbox-streets/${z}/${x}/${y}.png","http://c.tiles.mapbox.com/v3/mapbox.mapbox-streets/${z}/${x}/${y}.png","http://d.tiles.mapbox.com/v3/mapbox.mapbox-streets/${z}/${x}/${y}.png"],{numZoomLevels:18,attribution:otp.osm_att+"Tiles from<a href='http://mapbox.com/about/maps' target='_blank'> MapBox Streets.</a>"}),new OpenLayers.Layer.OSM("Open Street Map"),new OpenLayers.Layer.OSM("Open Cycle Map",["http://a.tile.opencyclemap.org/cycle/${z}/${x}/${y}.png","http://b.tile.opencyclemap.org/cycle/${z}/${x}/${y}.png","http://c.tile.opencyclemap.org/cycle/${z}/${x}/${y}.png"],{numZoomLevels:17,attribution:otp.osm_att+"and <a href='www.opencyclemap.org' target='_blank'>OpenCycleMap </a>"}),new OpenLayers.Layer.OSM("OSM MapQuest",["http://otile2.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png","http://otile3.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png","http://otile4.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png"],{sphericalMecator:true,isBaseLayer:true,numZoomLevels:19,attribution:otp.osm_att+" Tiles courtesy of <a href='http://open.mapquest.com/' target='_blank'>MapQuest</a>"}),new OpenLayers.Layer.OSM("Limited Zoom Example",["http://otile2.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png","http://otile3.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png","http://otile4.mqcdn.com/tiles/1.0.0/osm/${z}/${x}/${y}.png"],{serverResolutions:otp.ol_rez,maxResolution:76.43702827148437961569,sphericalMecator:true,isBaseLayer:true,numZoomLevels:8,attribution:otp.osm_att+" Tiles courtesy of <a href='http://open.mapquest.com/' target='_blank'>MapQuest</a>"})],baseLayerOptions:{projection:new OpenLayers.Projection("EPSG:4326"),url:'http://maps.opengeo.org/geowebcache/service/wms',layers:['openstreetmap'],format:'image/png',transitionEffect:'resize'}},attributionPanel:{enabled:false,panelTitle:otp.config.locale.config.attribution.title,attributionHtml:'<p class="disclaimer">'+otp.config.locale.config.attribution.content+'</p>'},splashScreen:{enabled:false,timeout:20,title:'Important: Please read',html:'<p class="splash-screen">'
